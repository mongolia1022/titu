...
<style type="text/css">
#stuffbox{ color:#999;}
#stuffbox .dttime{ font-size: 12px;}
...
<?php
	if(!empty($tag['data.results']))
	{
		...
		foreach($temResult as $data)
		{
			...
			{
					?>
					<td><span class="webSearch1"><a href="<?php echo sys_href($data['p'])?>"><h2><?php echo $data['title']; ?></h2></a></span></td>
					<?php
			}
			elseif($data['type'] == 'linkers')
			{
					?>
					<td><span class="webSearch1"><a href="<?php echo './?p='.$data['p'].'&r='.$data['id']; ?>"><h2><?php echo $data['title']; ?></h2></a></span></td>
					<?php 
			}
			...
			{
			    	?>
					<td><span class="webSearch1"><a href="<?php echo sys_href($data['p'],$data['type'],$data['id'])?>"><h2><?php echo $data['title']; ?></h2></a></span></td>
					<?php
			}
			?>
			</tr>
			...
			{
				?>
				<td class=dttime><?php echo $data['dtTime']; ?>&nbsp;&nbsp;<span class="webSearch2"><a href="<?php echo sys_href($data['menuName'])?>">快速链接</a></span></td>
				<?php 
			}elseif($data['type'] == 'linkers'){
				?>
				<td class=dttime><?php echo $data['dtTime']; ?>&nbsp;&nbsp;<span class="webSearch2"><a href="<?php echo '/?p='.$data['p'].'&r='.$data['id']; ?>">快速链接</a></span></td>
				<?php 
			}else{
				?>
				<td class=dttime><?php echo $data['dtTime']; ?>&nbsp;&nbsp;<span class="webSearch2"><a href="<?php echo sys_href($data['p'],$data['type'],$data['id'])?>">快速链接</a></span></td>
				<?php 
			}
			?>
			</tr>			
